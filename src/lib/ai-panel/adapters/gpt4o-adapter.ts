...
/**
 * Adapter for OpenAI GPT-4o (via the `openai` SDK).
 */
export class GPT4oAdapter extends BaseAdapter {
  ...
  async generate(request: AdapterRequest): Promise<AdapterResponse> {
    if (!this.isAvailable) {
      return this.handleError(new Error('OpenAI API key not configured'), this.config.model);
    }
    ...
    try {
      ...
      const costUsd = calculateCost('gpt4o', tokensInput, tokensOutput);
